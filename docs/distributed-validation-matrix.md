...
| QR para compartir la sala | Sin prueba automática dedicada actualmente. Demostración manual: `VITE_SERVER_URL=http://localhost:3003 npm --prefix client run dev` | Al crear una sala, la vista renderiza `QRCodeSVG` con la URL `/?room=<código>`. | El QR se ve y codifica la URL de la sala. Esta es una brecha de automatización, no un resultado de Cypress. |
